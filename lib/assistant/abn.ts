 ...
 * Chỉ chạy khi có ABN_LOOKUP_GUID (đăng ký miễn phí tại abr.business.gov.au,
 * mục Web Services) VÀ người dùng nhắc tới một ABN 11 chữ số.
 *
 * Dịch vụ JSON của ABR trả về dạng JSONP: callback({...}). parseAbnResponse bóc lớp đó.
 */
...
/** Kiểm tra ABN hợp lệ theo checksum của ATO (bắt lỗi gõ nhầm trước khi gọi API). */
...
export async function abnEvidence(abn: string | undefined): Promise<Evidence | null> {
  const guid = readEnv("ABN_LOOKUP_GUID");
