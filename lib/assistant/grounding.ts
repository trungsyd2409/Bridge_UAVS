 ...
 *   priority 3: RAG (rag-data.json -> hybrid search -> rerank)
...
async function timed<T>(step: string, trace: TraceStep[], fn: () => Promise<T>, detail?: (r: T) => unknown) {
  ...
  try {
    ...
    trace.push({ step, ms: Date.now() - t0, ok: true, detail: detail?.(result) });
    return result;
  } catch (e) {
    trace.push({ step, ms: Date.now() - t0, ok: false, detail: (e as Error).message });
    throw e;
  }
}
