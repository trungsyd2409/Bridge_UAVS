/**
 * NLU: phân tích câu hỏi của người dùng thành NluResult.
 *
 *   1. Làm sạch văn bản (Unicode NFC, cắt độ dài)
 *   2. Quét từ khoá khẩn cấp (không cần AI, không cần mạng)
 *   3. Gọi Gemini với responseSchema: intent, entities, risk, englishQuery
...
const EMERGENCY_KEYWORDS = [
  ...
  // Bỏng và điện giật ở bếp/xưởng là tai nạn lao động phổ biến nhất với nhóm người dùng này.
  ...
  // Bản tiếng Anh, cho người dùng đang để giao diện EN
  "held my passport", "threatened", "assaulted", "burned", "electric shock",
  "cannot leave", "locked in", "sexually assaulted", "kill myself", "want to die",
];
const SELF_HARM_KEYWORDS = ["tu tu", "muon chet", "tu sat", "kill myself", "want to die"];
...
const INTENT_KEYWORDS: Record<Exclude<Intent, "general">, string[]> = {
  ...
  underpayment: ["luong", "tien cong", "tra thieu", "tra it", "tien mat", "cash", "lam thu", "thu viec",
    ...
    "underpaid", "minimum wage", "overtime", "penalty rate"],
...
// Tài liệu trong kho là tiếng Anh: thiếu các từ này thì BM25 không khớp được.
...
const GLOSSARY: [string, string][] = [
  ...
  ["du hoc", "student visa work hours fortnight"], ["bao nhieu gio", "visa work hours limit fortnight"],
  ["duoi viec", "dismissal"], ["sa thai", "unfair dismissal"],
